  ...
  return (
    <div className="container">
      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '2rem' }}>Mouse<span style={{ color: '#38bdf8' }}>OJ</span></h1>
        <p className="text-muted">Cloudflare Workers + D1 + Hono — 零预算在线评测系统</p>
      </div>
        ...
        <div className="card">
          ...
          {data.announcements?.length ? data.announcements.map(a => (
            <div key={a.id} style={{ marginBottom: '0.5rem' }}>
              ...
              <div className="text-sm text-muted">{a.created_at}</div>
            </div>
          ...
          {data.upcomingContest ? (
            <div>
              ...
              <div className="text-sm text-muted">
                {new Date(data.upcomingContest.start_time).toLocaleString('zh-CN')}
              </div>
            </div>
